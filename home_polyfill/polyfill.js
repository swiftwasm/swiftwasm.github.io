...
var ASM_CONSTS = [function() { const imports = { wasi_unstable: WASIPolyfill };         WebAssembly.instantiate(wasi_wasm_buffer, imports)         .then(obj => {             setInstance(obj.instance);             try {                 obj.instance.exports._start();             } catch (e) {                 if (e instanceof WASIExit) {                     handleWASIExit(e);                 } else {                     console.log(e);                     if (window.wasi_handle_error) window.wasi_handle_error(e);                 }             }         })         .catch(error => {             console.log('error! ' + error);         }); }];
